...
const globalLeaderboardKey = 'global'

/*
leaderboard functionality
 * ability to look up top spot
 * ability to read entire leaderboard
 * custom leaderboard - tell friends to join
 * self only leaderboard
 * ability to input name into leaderboard
 * loserboard
*/
  ...
  handle(handlerInput) {
    return (
      handlerInput.responseBuilder
        .speak("Welcome to the dice roll game. This game is all about testing your luck. Would you like to play the game? or would you like to view the leaderbaord?")
        .reprompt("Sorry I didn't get that. Would you like to play or view the leaderboard?")
        .getResponse()
    );
  }
};
...
const rollDice = function() {
  //TODO should we allow for different sided die?
  return Math.floor(Math.random() * 5) + 1;
};

const StartGameRequestHandler = {
  canHandle(handlerInput) {
    const request = handlerInput.requestEnvelope.request;
    return (
      request.type === "IntentRequest" &&
      request.intent.name === "PlayDiceRollIntent"
    );
  },
  handle(handlerInput) {
    ...
    var prompt = "Starting new game. ";
    if(badLuck) {
      prompt += "Your first roll was 1 but luckily we are nice and re-rolled for you. Now "
    }
    prompt += "Your first roll is " + diceRoll + ", which is also your current score. Would you like to roll again or immortalize your new score on the leaderboard?"
    sessionAttributes["hasGame"] = true
    sessionAttributes["score"] = diceRoll
    handlerInput.attributesManager.setSessionAttributes(sessionAttributes);

    return (
      handlerInput.responseBuilder
        .speak(prompt)
        .reprompt("hello?") //todo clean up message
        .getResponse()
    );
  }
};

// const LeaderboardRequestHanlder = {
//   canHandle(handlerInput) {
//     const request = handlerInput.requestEnvelope.request
//     return (
//       request.type === "IntentRequest" &&
//       request.intent.name === "RollDiceIntent"
//     );
//   },
//   handle(handlerInput) {
...
//       Key: { Name: globalLeaderboardKey }
//     })
//   }
// }

const RollDiceRequestHandler = {
  canHandle(handlerInput) {
    const request = handlerInput.requestEnvelope.request;
    return (
      request.type === "IntentRequest" &&
  ...
  handle(handlerInput) {
    ...
    const sessionAttributes = handlerInput.attributesManager.getSessionAttributes();
    const hasGame = sessionAttributes["hasGame"];
    if (!hasGame) {
      // if don't have game we should start a new game for user
      return StartGameRequestHandler.handle(handlerInput);
    }
    ...
    if (diceRoll === 1) {
      ...
      score = 0
      // TODO maybe tease with "if you would have stopped you could have been position X on leaderboard"
      var prompt = "Uh oh! It looks like you rolled a 1. This ends your game. Better luck next time. Would you like to play again?";
    } else {
      score += diceRoll
    }
    sessionAttributes["score"] = score;
    handlerInput.attributesManager.setSessionAttributes(sessionAttributes);
    prompt = "You rolled a " + diceRoll + ". Your current score is now " + score + ". Would you like to roll again or immortalize your new score on the leaderboard?"
